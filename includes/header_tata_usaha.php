...
// pastikan hanya tata usaha yang bisa akses
if (!isset($_SESSION['user_id']) || $_SESSION['role'] != 'tata_usaha') {
...
<head>
  ...
  <style>
    ...
    .avatar { width: 34px; height: 34px; display: inline-flex; align-items: center; justify-content: center;
              background: #fff; color: #17a2b8; font-weight: 700; font-size: 1rem; border-radius: 50%; }
    .dropdown-menu { border: none; border-radius: .75rem; box-shadow: 0 8px 24px rgba(0,0,0,.15); }
    .dropdown-menu .dropdown-header { color: #17a2b8; font-weight: 600; }
  </style>
</head>
<body>
...
<nav class="navbar navbar-expand-lg navbar-dark navbar-custom sticky-top">
  <div class="container-fluid">
    <a class="navbar-brand d-flex align-items-center" href="<?= $base_url ?>/tata_usaha/index.php">
    ...
    <div class="collapse navbar-collapse" id="navbarNav">
      <ul class="navbar-nav mx-auto mb-2 mb-lg-0">
        <li class="nav-item">
          <a class="nav-link <?= basename($_SERVER['PHP_SELF'])=='index.php'?'active':'' ?>" href="<?= $base_url ?>/tata_usaha/index.php"><i class="fa fa-gauge-high"></i>Dashboard</a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= basename($_SERVER['PHP_SELF'])=='surat_masuk.php'?'active':'' ?>" href="<?= $base_url ?>/tata_usaha/surat_masuk.php"><i class="fa fa-inbox"></i>Surat Masuk</a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= basename($_SERVER['PHP_SELF'])=='surat_keluar.php'?'active':'' ?>" href="<?= $base_url ?>/tata_usaha/surat_keluar.php"><i class="fa fa-paper-plane"></i>Surat Keluar</a>
        </li>
        <li class="nav-item">
          <a class="nav-link <?= basename($_SERVER['PHP_SELF'])=='disposisi.php'?'active':'' ?>" href="<?= $base_url ?>/tata_usaha/disposisi.php"><i class="fa fa-tasks"></i>Disposisi</a>
        </li>
      </ul>
      ...
      <ul class="navbar-nav ms-lg-3">
        <li class="nav-item dropdown">
          <a class="nav-link dropdown-toggle d-flex align-items-center gap-2" href="#" id="userDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false">
            ...
            <span><?= htmlspecialchars($nama_user); ?> <small class="text-white-50">(Tata Usaha)</small></span>
          </a>
          <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="userDropdown">
            <li><h6 class="dropdown-header"><i class="fa fa-circle-user me-1"></i><?= htmlspecialchars($nama_user); ?></h6></li>
            <li><hr class="dropdown-divider"></li>
            <li><a class="dropdown-item" href="<?= $base_url ?>/tata_usaha/profil.php"><i class="fa fa-user me-2"></i>Profil</a></li>
